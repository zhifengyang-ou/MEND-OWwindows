...
!        write(*,*)'SSE=',m,sum1,AVGobs,var1
    ...
    REAL(8) function f1RAVG_ratio(nn, dobs, dsim, ratio,dFill)
        ...
        !!LOCAL VARIABLES:
        REAL(8) fAVG_obs, fAVG_sim, dx_obs,dx_sim
		INTEGER k, jua

!! change due to average value will be calculated for all data points, including values outsied simulation period
        fAVG_obs = 0.0
		fAVG_sim =0.0
        k = 0
        do jua = 1, nn
            if ((dobs(jua) .ne. dFill).and.(dsim(jua) .ne. dFill)) then
                k = k + 1
                if(abs(dobs(jua)).ge.dHIGH.or.abs(dobs(jua)).le.dLOW) then  !!wgs [12/12/2018]: avoid big/small values
                    dx_obs = 0.d0 
                else
                    dx_obs = dobs(jua)
                end if
				if(abs(dsim(jua)).ge.dHIGH.or.abs(dsim(jua)).le.dLOW) then  !!wgs [12/12/2018]: avoid big/small values
                    dx_sim = 0.d0 
                else
                    dx_sim = dsim(jua)
                end if
                fAVG_obs = fAVG_obs + dx_obs !!min(dHIGH,max(dLOW,xx(jua)))
				fAVG_sim = fAVG_sim + dx_sim
            end if
        end do
        if (k < 1) then
            fAVG_obs = dFill
